...
namespace _1_Presentacion
{
    public partial class Creación_Actividades : Form
    {
        ...
        public void Cargar_Contricante1(int idContricante_1)
        {
            try
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("SELECT idCategoria,Nombre FROM TBL_Categoria WHERE idDeporte = @idDeporte", con);
                cmd.Parameters.AddWithValue("idDeporte", idContricante_1);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
                da.Fill(dt);
                con.Close();
        ...
        {
            string query = "INSERT INTO TBL_ACTIVIDAD (idDeporte,Nombre,idCategoria,Fecha) VALUES (@deporte,@nombre,@categoria,@fecha)";
            con.Open();
